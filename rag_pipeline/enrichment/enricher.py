...
from rag_pipeline.enrichment.json_utils import repair_and_load_json, parse_enrichment_data, clean_llm_json
from rag_pipeline.core.prompts import ENRICH_PROMPT
from rag_pipeline.enrichment.enrichment_validator import is_corrupted_output, is_low_quality_enrichment
...
class ChunkEnricher:
    ...
    def enrich_chunk(self, chunk: Chunk) -> Tuple:
        ...
        """
        content_preview = get_compact_content(chunk)
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    from rag_pipeline.enrichment.json_utils import log_malformed_json
    ...
    def _enrich_chunk_with_model_internal(self, chunk: Chunk, model: str) -> Tuple:
        ...
        prompt = ENRICH_PROMPT.format(
            content=get_compact_content(chunk),
            max_questions=self.config.max_questions
        )
